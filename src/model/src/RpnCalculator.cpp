...
RpnCalculator::RpnCalculator(std::vector<Symbol> input) : 
	symbols{input}, current_iterator{symbols.begin()}
{
}

OperationResult RpnCalculator::do_calculate_next()
{
	if (has_finished())
	{
...
bool RpnCalculator::do_has_finished() const
{
	return current_iterator == symbols.end();
}

void RpnCalculator::extract_and_insert_values()
{
...
void RpnCalculator::process_operation()
{
	if (not has_finished())
	{
